...
print('=' * 60)
print('📊 반디 퀀트 - 한미반도체 (042700.KS) 단독 분석')
print('=' * 60)
print(f'⏰ {datetime.now().strftime("%Y-%m-%d %H:%M")} KST')
print()

# 데이터 수집
df = yf.download('042700.KS', period='6mo', progress=False)
...
print(f'💰 현재가: {current:,.0f}원 (전일대비 {change_pct:+.2f}%)')
print()
...
print(f'📈 20일 고가: {high_20:,.0f}원')
print(f'📉 20일 저가: {low_20:,.0f}원')
print()
...
for i in range(3, 0, -1):
    ...
    print(f'   {date}: {color} | {o:,.0f}원 → {c:,.0f}원')
...
if CHART_AVAILABLE:
    ...
    # 차트 파일명 (한글 이름 처리)
    safe_name = "한미반도체"
    chart_path = f"{CHART_DIR}/{safe_name}_단독분석_{date_str}.png"
